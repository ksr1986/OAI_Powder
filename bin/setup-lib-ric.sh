...
#
# -----------------------------------------------------------------------
# OAI_Powder RIC overrides (adapted from oran profile setup-lib.sh).
# This file is sourced by setup-kubespray-ric.sh, setup-oran-ric.sh,
# and setup-kubernetes-extra-ric.sh on the cn5g node only.
# The cn5g node co-hosts Open5GS (installed by deploy-open5gs.sh) and
# a single-node Kubernetes cluster running the OSC near-RT RIC (j-release).
# The OAI_shared_VLAN (192.168.1.0/24) carries N2/N3 and E2 traffic
# between cn5g and cudu (OAI DU).
# -----------------------------------------------------------------------

# cn5g is the RIC/Kubernetes head node in this profile.
HEAD="cn5g"
DO_APT_INSTALL=1
DO_APT_UPGRADE=0
...
KUBEENABLEMULTUS=0
KUBEPROXYMODE="ipvs"
# CRITICAL: must not overlap with OAI_shared_VLAN subnet (192.168.1.0/24).
KUBEPODSSUBNET="10.233.0.0/16"
...
SSLCERTTYPE="self"
SSLCERTCONFIG="proxy"
# OAI_shared_VLAN is the link name in the Emulab manifest for the
# cn5g <-> cudu link.  /etc/hosts will have entries like cn5g-OAI-shared-vlan.
MGMTLAN="OAI-shared-vlan"
DATALAN="OAI-shared-vlan"
DATALANS="OAI-shared-vlan"
# cn5g's IP on the OAI_shared_VLAN (192.168.1.1).
SINGLENODE_MGMT_IP=192.168.1.1
SINGLENODE_MGMT_NETMASK=255.255.255.0
SINGLENODE_MGMT_NETBITS=24
SINGLENODE_MGMT_CIDR=${SINGLENODE_MGMT_IP}/${SINGLENODE_MGMT_NETBITS}
DOLOCALREGISTRY=1
STORAGEDIR=/storage
# NFS not needed for the minimal OSC RIC deployment.
DONFS=0
NFSEXPORTDIR=$STORAGEDIR/nfs
NFSMOUNTDIR=/nfs
NFSASYNC=0
YQFROMPKG=0
# Open5GS is installed separately by deploy-open5gs.sh; do not reinstall here.
INSTALLOPEN5GS=0
# No RAN software on the RIC node.
BUILDSRSRANPROJECT=0
SRSRANPROJECTREPO=https://github.com/srsRAN/srsRAN_Project
SRSRANPROJECTREFSPEC=main
BUILDOCUDU=0
OCUDUREPO=https://gitlab.com/ocudu/ocudu
OCUDUREFSPEC=release_26_04
BUILDSRSRAN4G=0
SRSRAN4GREPO=https://github.com/srsRAN/srsRAN_4G
SRSRAN4GREFSPEC=master
BUILDSRSLTE=0
BUILDOAI=0
DOKPIMONGODEPLOY=0
DOKPIMONDEPLOY=0
...
RICIRELEASE=9
RICJRELEASE=10
RICVERSION=$RICJRELEASE
RICRELEASE="j-release"
INSTALLORANSC=1
INSTALLONFSDRAN=0
ONFRICVERSION="1.4.3"
ONFRICPOWDER=0
INSTALLORANSCSMO=0
OSCSMOVERSION="g-release"
OSCSMOUSECACHEDCHARTS=1
INSTALLORANSCSMOSIM=0
...
if [ ! -e $OURDIR/encrypted_admin_pass ]; then
    cat $OURDIR/manifests.0.xml | perl -e '@lines = <STDIN>; $all = join("",@lines); if ($all =~ /^.+<[^:]+:password[^>]*name="adminPass"[^>]*>([^<]+)<\/[^:]+:password>.+/igs) { print $1; }' > $OURDIR/encrypted_admin_pass
fi
...
RICSHORTRELEASE=$RICRELEASE
if [ "$RICRELEASE" = "dawn" ]; then
    RICVERSION=$RICDAWN
elif [ "$RICRELEASE" = "e-release" ]; then
    RICVERSION=$RICERELEASE
    RICSHORTRELEASE="e"
elif [ "$RICRELEASE" = "f-release" ]; then
    RICVERSION=$RICFRELEASE
    RICSHORTRELEASE="f"
elif [ "$RICRELEASE" = "g-release" ]; then
    RICVERSION=$RICGRELEASE
    RICSHORTRELEASE="g"
elif [ "$RICRELEASE" = "h-release" ]; then
    RICVERSION=$RICHRELEASE
    RICSHORTRELEASE="h"
elif [ "$RICRELEASE" = "i-release" ]; then
    RICVERSION=$RICIRELEASE
    RICSHORTRELEASE="i"
elif [ "$RICRELEASE" = "j-release" ]; then
    RICVERSION=$RICJRELEASE
    RICSHORTRELEASE="j"
fi
...
# OVERRIDE: The fqdn.map contains all experiment nodes (cn5g, cudu, ru1, ue1).
# Kubernetes must be installed ONLY on cn5g (single-node cluster).
# Force NODES and NODECOUNT to just the cn5g node.
NODES="cn5g"
FQDNS=`cat $OURDIR/fqdn.map | grep "^cn5g" | cut -f2`
NODEIPS="$SINGLENODE_MGMT_IP"
NODECOUNT=1
